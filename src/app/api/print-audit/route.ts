    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="number prev-read">${r.prevRead?.toLocaleString() || '-'}</td>
          <td>${r.prevDate || '-'}</td>
          <td class="number curr-read">${r.currentRead?.toLocaleString() || '-'}</td>
          <td>${r.currentDate || '-'}</td>
          <td class="${r.diff && r.diff < 0 ? 'diff-negative number' : 'number'}">${r.diff?.toLocaleString() || '-'}</td>
          <td class="number">${r.days || '-'}</td>
          <td class="number daily-rate">${r.dailyRate ? Math.round(r.dailyRate) : '-'}</td>
          <td class="number rate1">${r.rate1 ? Math.round(r.rate1) : '-'}</td>
          <td class="number rate2">${r.rate2 ? Math.round(r.rate2) : '-'}</td>
          <td class="number rate3">${r.rate3 ? Math.round(r.rate3) : '-'}</td>
          <td>${r.enterName || '-'}</td>
          <td>
